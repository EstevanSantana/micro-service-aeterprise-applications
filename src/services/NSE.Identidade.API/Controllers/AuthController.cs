...
namespace NSE.Identidade.API.Controllers
{
    ...
    public class AuthController : MainController
    {
        ...
        public async Task<IActionResult> Login(UsuarioLogin usuarioLogin) 
        {
            ...
            {
                AdicionarErroProcessamento("Usuario temporariamente bloquado");
                return CustomResponse();
            }
        ...
        private async Task<UsuarioRespostaLogin> GerarToken(string email)
        {
            ...
            return ObterResponseToken(encodedToken, user, claims);
        }

        private UsuarioRespostaLogin ObterResponseToken(string encodedToken, IdentityUser user, IEnumerable<Claim> claims)
        {
            return new UsuarioRespostaLogin()
            {
                ...
                UserToken = new UsuarioToken
                {
                    Id = user.Id,
                    Email = user.Email,
                    Claims = claims.Select(c => new UsuarioTokenClaim { Type = c.Type, Value = c.Value })
                }
            };
        }
        ...
        private async Task<ClaimsIdentity> ObterClaimsUsuario(ICollection<Claim> claims, IdentityUser user)
        {
            ...
            var identiryClaims = new ClaimsIdentity();
            identiryClaims.AddClaims(claims);

            return identiryClaims;
        }
