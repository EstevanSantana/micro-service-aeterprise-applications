...
namespace NSE.Carrinho.API.Controllers
{
    ...
    public class CarrinhoController : MainController
    {
        ...
        {
            return Ok(await ObterCarrinhoCliente() ?? new CarrinhoCliente());
        }
        ...
        {
            var carrinho = await ObterCarrinhoCliente();
            var itemCarrinho = await ObterItemCarrinhoValido(produtoId, carrinho, item);
            if (itemCarrinho == null) return CustomResponse();

            carrinho.AtualizarUnidade(item, item.Quantidade);
        ...
        public async Task<IActionResult> RemoverItemCarrinho(Guid produtoId)
        {
            ...
            var itemCarrinho = await ObterItemCarrinhoValido(produtoId, carrinho);
            if (itemCarrinho == null) return CustomResponse();
        ...
        private void ManipularCarrinhoExistente(CarrinhoCliente carrinho, CarrinhoItem item)
        {
            ...
            {
                _context.CarrinhoItens.Update(carrinho.ObterCarrinhoPorId(item.ProdutoId));
            }
            else
            {
        ...
        private async Task<CarrinhoItem> ObterItemCarrinhoValido(Guid produtoId, CarrinhoCliente carrinho, CarrinhoItem item = null)
        {
